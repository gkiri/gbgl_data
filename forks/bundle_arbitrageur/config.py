...
BA_MAX_BUNDLE_COST = float(os.getenv("BA_MAX_BUNDLE_COST", "1.02"))      # Default stays strict; override in .env for A/B
BA_TARGET_BUNDLE_COST = float(os.getenv("BA_TARGET_BUNDLE_COST", "0.985"))  # Was 0.97, realistic target
BA_ABORT_BUNDLE_COST = float(os.getenv("BA_ABORT_BUNDLE_COST", "1.002"))   # Tighter - losses happen >1.00
...
BA_MIN_POSITION_SIZE = float(os.getenv("BA_MIN_POSITION_SIZE", "0"))   # Don't bother with tiny positions
BA_MAX_TOTAL_EXPOSURE = float(os.getenv("BA_MAX_TOTAL_EXPOSURE", "3000"))   # Max capital at risk (kept very small for tests)
...
BA_MAX_NO_PRICE = float(os.getenv("BA_MAX_NO_PRICE", "0.99"))            # Require a cheap hedge leg (observed 0.02-0.29)
...
BA_FILLS_PER_SWEEP = int(os.getenv("BA_FILLS_PER_SWEEP", "20"))           # Very small burst for testing
...
BA_SPREAD_MIN = float(os.getenv("BA_SPREAD_MIN", "0.0"))                       # require at least this spread on each leg
BA_DEPTH_MIN = float(os.getenv("BA_DEPTH_MIN", "1.0"))                            # min depth (shares) at touch per leg
...
BA_CLIP_LADDER = tuple(float(x) for x in os.getenv("BA_CLIP_LADDER", "10,12,14,16").split(","))
BA_BURST_CHILD_COUNT = int(os.getenv("BA_BURST_CHILD_COUNT", "10"))             # orders per trigger burst (pairs or singles)
