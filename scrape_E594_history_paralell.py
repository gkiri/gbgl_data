...
Usage:
    python scrape_E594_history_paralell.py --days 30 --workers auto
    python scrape_E594_history_paralell.py --days 7 --workers 16 --batch-size 50
    python scrape_E594_history_paralell.py --days 1 --workers 4  # Test run

    python scrape_E594_history_paralell.py --days 30 --workers auto --asse ETH
"""
...
# CONFIG
# ------------------------------------------------------------------------------
GABAGOOL_ADDRESS = "0xe594336603f4fb5d3ba4125a67021ab3b4347052"
...
SOL_SLUG_PREFIX = "sol-updown-15m"
XRP_SLUG_PREFIX = "xrp-updown-15m"
...
def generate_market_tasks(days: int = 30, asset: str = "BTC") -> List[MarketTask]:
    ...
    asset_u = asset.upper()
    try:
        prefix = SLUG_PREFIX_BY_ASSET[asset_u]
    except KeyError:
        raise ValueError(f"Unsupported asset '{asset}'. Supported: {', '.join(SLUG_PREFIX_BY_ASSET.keys())}")
